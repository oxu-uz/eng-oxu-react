...
function Section1(props) {
  ...
  const slides = [
    {
      ...
      poster: "/pic4-01.jpg",
      title:
        "Welcome to AIU. Ready to start on an exciting journey with us? Explore our university and become part of the family.",
      description:
  ...
  const remainingText = slides[currentIndex].title
    .split(".")
    .slice(1)
    .join(".")
    .trim();
      ...
      <div>
                  ...
                  {item.type === "logo" ? (
                    ...
                    >
                      <MenuHandler>
                        <div
                          className={`${
                            isNavHovered || isMenuOpen
                              ? "text-white"
                              : "text-white"
                          } hover:text-white nav-link uppercase flex cursor-pointer gap-3 shadow-none outline-none items-center px-4 py-2 top-nav-sup-link focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-600 transition-colors duration-300 text-lg`}
                        >
                          {item.label}{" "}
                        ...
                        {item.dropdown.map((dropdownItem, i) => (
                          ...
                          >
                            {dropdownItem.submenu ? (
                              ...
                              >
                                <MenuHandler>
                                  <MenuItem className="hover:bg-blue-50 group flex justify-between items-center transition-colors duration-300">
                                    {dropdownItem.label}{" "}
                                    <ChevronRight
                              ...
                              >
                                <MenuItem className="text-white hover:bg-blue-50 link transition-colors duration-300">
                                  {dropdownItem.label}
                                </MenuItem>
                              </a>
                            )}
                    ...
                    <a
                      href={item.link}
                      className={`${
                        isNavHovered || isMenuOpen ? "text-white" : "text-white"
                      } hover:text-white uppercase nav-link flex items-center shadow-none outline-none ring-0 px-4 py-2 top-nav-sup-link focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-600 transition-colors duration-300 text-lg`}
                    >
                      {item.label}
                    </a>
                  )}
        ...
        <div className="relative w-full min-h-[100vh] overflow-hidden">
          <div className="absolute inset-0">
            <div className="relative flex h-full items-center justify-center w-full z-20 text-[#012152] py-6 md:py-0">
              <div className="text-left bg-white opacity-80 p-6 md:p-8 rounded-lg">
                {/* Welcome Text with Backspace Animation */}
                <div className="flex items-center">
                  <motion.span className="text-2xl md:text-4xl font-extrabold uppercase">
                    {displayText}
                  </motion.span>
                  <motion.span
