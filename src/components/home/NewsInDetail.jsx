            ...
            <aside className="hidden md:w-1/3 lg:w-1/3 py-4 md:block">
                <div className="sticky top-12 flex flex-col gap-2 p-2 rounded-xl">
                    <div className="section-header-title pl-3">
                        <h2 className="section-title text-[#0a37b3] uppercase text-3xl font-black">YANGILIKLAR</h2>
                            ...
                            <div className="flex flex-col gap-3 max-h-[70vh] overflow-y-auto pr-2">
                                {announcements.map((item, index) => (
                                    <Link
                                        to={`/news/${item.id}`}
                                        key={item.id}
                                        className={`p-3 rounded-md border transition duration-200 bg-white hover:bg-[#f0f9ff] hover:border-[#0a37b3] hover:shadow-lg hover:scale-[0.97]`}>
